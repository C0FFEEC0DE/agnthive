...
This plan converts `agnthive` from a copied `~/.claude` profile
...
The target plugin identifier is **`agnthive`**. Confirm that the
...
`"source": "./plugins/agnthive"`. A self-hosted marketplace is
...
3. **Create the marketplace and plugin manifests.**
   ...
   - Add `.claude-plugin/marketplace.json` and
     `plugins/agnthive/.claude-plugin/plugin.json` with SPDX
   ...
   - Verify: `claude plugin validate . --strict` and
     `claude plugin validate plugins/agnthive --strict` pass.
