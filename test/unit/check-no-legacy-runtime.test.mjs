...
function makeTree(root) {
  ...
  mkdirSync(join(root, 'plugins', 'agnthive', 'scripts'), { recursive: true });
  writeFileSync(join(root, 'plugins', 'agnthive', 'scripts', 'statusline.mjs'), '// node\n');
...
test('fails when a stray .sh is placed under plugins/', () => {
  ...
  try {
    makeTree(root);
    writeFileSync(join(root, 'plugins', 'agnthive', 'legacy.sh'), '#!/bin/bash\nexit 0\n');
    const r = checkNoLegacyRuntime(root);
...
test('fails when a stray .py appears under plugins/', () => {
  ...
  try {
    makeTree(root);
    writeFileSync(join(root, 'plugins', 'agnthive', 'helper.py'), 'print(1)\n');
    const r = checkNoLegacyRuntime(root);
    assert.equal(r.ok, false);
