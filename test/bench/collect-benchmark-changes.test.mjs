...
test('workflow_dispatch on feature branch collects diff vs base', () => {
  ...
  writeFile(join(worktree, 'plugins/agnthive/agents/manager.md'), 'changed\n');
  run('git', ['add', 'plugins/agnthive/agents/manager.md'], { cwd: worktree });
  ...
  assert.deepEqual(readFileSync(output, 'utf-8').split('\n').filter(Boolean), ['plugins/agnthive/agents/manager.md']);
});
